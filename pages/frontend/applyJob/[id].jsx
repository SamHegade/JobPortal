...
import { apply_job } from '../../../Services/job/index';
...
export default function ApplyJob() {
    ...
    const [formikData, setFormikData] = useState({ name: activeUser?.name,mothername: '',fathername: '', email: activeUser?.email , countryCode:'',contact:'',about: '', job: id,qualification:'',address:'',dob:'',gender:'',marks:'',year:'', cv: '' ,user: activeUser?._id })
    const [file, setFile] = useState(null)
    const [error, setError] = useState({ name: '',mothername: '',fathername: '', email: "",countryCode:'',contact:'', about: '', job: '',qualification:'',address:'',dob:'',gender:'', marks:'',year:'', cv: '',user: '' });


    const { name,mothername,fathername ,email,countryCode,contact, about, job, user,qualification,address,gender,dob ,marks,year,cv} = formikData;
    ...
    const handleSubmit = async (e) => {
        ...
        if (!countryCode) {
            setError({ ...error, countryCode: "countryCode Field is required" })
            return;
        }
        if (!contact) {
            setError({ ...error, contact: "countryCode Field is required" })
            return;
        }
        ...
        <>
            ...
            <div className='w-full  py-20 flex items-center  justify-center flex-col'>
                ...
                <form encType="multipart/form-data" onSubmit={handleSubmit} className="sm:w-1/2 w-full px-4 mx-4  h-full" >
                    ...
                    <div className='w-full mb-4 flex flex-col items-start justify-center'>
    <label htmlFor="contact" className='mb-1 text-base font-semibold'>Contact :</label>
    <div className="flex">
        <input
            name='countryCode'
            onChange={(e) => setFormikData({ ...formikData, countryCode: e.target.value })}
            type="text"
            id='countryCode'
            className='w-1/4 py-2 px-3 mb-2 border border-indigo-600 rounded'
            placeholder='Country'
        />
        <input
            name='contact'
            onChange={(e) => {
                ...
                if (numericValue.length >= 10) {
                    setFormikData({ ...formikData, contact: numericValue });
                }
            }}
            type="text"
            pattern="[0-9]*"
            id='contact'
            className='w-3/4 py-2 px-3 mb-2 border border-indigo-600 rounded'
    ...
    </div>
    {
        error.contact && <p className="text-sm text-red-500">{error.contact}</p>
                    ...
                    <div className='w-full mb-4  flex flex-col items-start justify-center'>
                        <label htmlFor="marks" className='mb-1 text-base text-white font-semibold'>Passing marks :</label>
