...
export default function  Register (){
  ...
  const [error, setError] = useState({ email: "", password: "", name: '',usertype:"" });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.usertype) {
      setError({ ...error, usertype: "usertype Field is required" })
      return;
    }
        ...
        <div className="w-full bg-white rounded-lg shadow dark:border text-black md:mt-0 sm:max-w-md xl:p-0 ">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6" action="#">
              ...
              <Select onChange={(e) => setFormData({ ...formData, usertype: e.value })} placeholder="Please Select usertype" type="Usertype" id='usertype' options={options} />
                    <div className='w-full mb-4  flex flex-col items-start justify-center'>
                        {
                            error.usertype && <p className="text-sm text-red-500">{error.usertype}</p>
                        }
                    </div>
