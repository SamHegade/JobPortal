...
export default function NavBar() {
    ...
    // useEffect(() => {
    //     dispatch(setUserType(localStorage.getItem('usertype') ? JSON.parse(localStorage.getItem('usertype')) : null));
    // }, [dispatch])
    ...
    return (
        <>
         <div className="px-2 h-full flex items-center justify-center">
            ...
            <div className={`w-full ${scrolled ? "bg-indigo-600/70" : "bg-blue-600"} px-8 h-19 bg-indigo-600 text-white flex items-center justify-between fixed top-0 left-0 z-50 x-10`} style={{paddingBottom:'10px',paddingTop:'15px'}}>
                <div className='px-2 h-full flex items-center justify-center'>
                    <p className='uppercase font-bold tracking-widest text-lg'>DREAM_BRIGHT</p>
                    {/* <img src={"./logo3.jpg"} alt="Logo" className="h-9 transparent" /> Replace 'h-8' with the appropriate height for your logo */}
                ...
                <div className='px-2 h-full hidden items-center justify-center lg:flex ' >
                    {
                        user?.usertype ===  "user" ?(
                            <>                <Link href={'/'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Home</Link>
                                            <Link href={'/frontend/displayJobs'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Apply to Jobs</Link> 
                                            ...
                                            <Link href={'/frontend/aboutUs'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >About us</Link>
                            </>
                        ):null
                    }
                    </div>
                    <div>
                        ...
                        user?.usertype ===  "recruiter"?(
                            <>                      
                                            <Link href={'/'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Home</Link>
                                                    <Link href={'/frontend/postAJob'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Post A Job</Link>
                                                    <Link href={'/frontend/postedJob'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Applicants</Link>
                                                    <Link href={'/frontend/aboutUs'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >About us</Link>
                ...
                <div>
                        ...
                        user?.usertype ===  "admin"?(
                            <>                                      <Link href={'/'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Home</Link>
                                                    <Link href={'/frontend/jobDetails2/${job?._id}'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Jobs</Link>
                                                    <Link href={'/frontend/adminApplications/${job?._id}'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Applicants</Link>
                            ...
                            <div className='px-2 h-full  items-center justify-center flex' >
                                {
                                    user !== null ? (
                                        <>
                                            ...
                                            <BiLogOut onClick={handleLogout} className=' cursor-pointer text-3xl hover:text-red-500 transition-all duration-700' />
                                            <p className='text-lg px-4 font-semibold'>{user?.name}{user?.usertype}</p>
                                            <div className='px-2 h-full hidden items-center justify-center lg:flex'>
                                            {/* Add the profile link here */}
                                           {/* <Link href={'/frontend/profile'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Profile</Link> */}
                                           </div>
