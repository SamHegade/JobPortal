...
export default function AppliedJobDataTable() {
    ...
    const columns = [
        ...
        {
            name: 'Action',
            cell: row => <button onClick={() => router.push(`/frontend/jobDetails/${row?.job?._id}`)} className='md:px-2 md:py-2 px-1 py-1 text-xs text-indigo-600 hover:text-white my-2 hover:bg-indigo-600 border border-indigo-600   rounded transition-all duration-700  '>view job Detail</button>
        },
    ];
